...
<div class="page-content container container-plus">
	...
	<div class="card-body px-3 pb-1">
		<form class="mt-lg-3" autocomplete="off" method="post" action="{{route('pembelian.store')}}">
	    @csrf
		...
		<div class="form-group row">
			...
			<div class="col-sm-9">
			 	<select class="form-control col-sm-8 col-md-6" id="supplier" name="supplier" required="true">
			 		...
			 		<option value="{{$supplier->IdSupplier}}"{{$key==0?' selected':''}}>{{$supplier->NamaSupplier}}</option>
			 		@endforeach
			 	</select>
			</div>
		</div>
